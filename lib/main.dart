...
class _LoginPageState extends State<LoginPage> {
  ...
  // Function to fetch user role from public.users table
  Future<String?> getUserRole(String email) async {
    try {
      ...
      final response = await supabase
          ...
          .eq('email', email)  // Query by email instead of ID
          .single();

      print("User role query result: ${response.toString()}");
      return response['role'] as String?;
    } catch (e) {
      print("Error fetching user role from users table: $e");
      return null;
    }
  }

  // Helper function to normalize role (handle case and whitespace)
  ...
  Future<void> loginUser() async {
    ...
    try {
      ...
      if (response.user != null) {
        ...
        // Get the user's role from the public.users table using email
        final role = await getUserRole(response.user!.email!);

        print("Raw role from database: '$role'");
        print("Normalized role: '${normalizeRole(role)}'");
        ...
        // Redirect based on user role (using normalized comparison)
